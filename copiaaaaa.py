...
if df is not None:
    ...
    falta_columna = any(columna not in df.columns for columna in columnas_requeridas)
    
    if falta_columna:
        st.error("❌ Faltan columnas requeridas en el archivo Excel. Verifica los nombres.")
    else:
        ...
        if not st.session_state.juego_iniciado:
            ...
            if not niveles_disponibles:
                st.error("❌ No se encontraron los niveles adecuados en tu Excel.")
            else:
        ...
        # --- PANTALLA DE JUEGO ---
            ...
            if idx >= total_preguntas:
                ...
                # Opciones mapeadas
                opciones_texto = [f"A) {fila['A']}", f"B) {fila['B']}", f"C) {fila['C']}", f"D) {fila['D']}"]
                mapa_letras = {"A": opciones_texto[0], "B": opciones_texto[1], "C": opciones_texto[2], "D": opciones_texto[3]}
                
                # Renderizar los controles de respuesta de manera nativa sin formularios complejos
                if not st.session_state.respondido:
                    seleccion = st.radio("Selecciona tu respuesta:", opciones_texto, key=f"radio_{idx}")
                    
                    if st.button("Enviar Respuesta", use_container_width=True, key=f"btn_{idx}"):
                        st.session_state.respondido = True
                        letra_seleccionada = seleccion[0] # Toma la primera letra 'A', 'B', 'C' o 'D'
                ...
                # Mostrar retroalimentación si ya se respondió
                    ...
                    correcta = str(fila["respuesta"]).strip().upper()
                    texto_seleccionado = mapa_letras.get(correcta, "N/A")
                    
                    st.disabled = True
                    st.radio("Tu respuesta seleccionada:", opciones_texto, index=opciones_texto.index(mapa_letras.get(str(fila["respuesta"]).strip().upper(), opciones_texto[0])), disabled=True, key=f"radio_dis_{idx}")

                    if st.session_state.respuesta_correcta:
                        st.success("✅ ¡Correcto!")
                    else:
                        st.error(f"❌ Incorrecto. La respuesta correcta es: {texto_seleccionado}")
                    
                    st.markdown(f"**💡 Información:**\n\n{fila['explicación']}")
                    st.markdown("---")
                    
                    if st.button("Siguiente Pregunta ➡️", use_container_width=True, key=f"next_{idx}"):
